...
expect(Number.isFinite(metrics.quality.catalog),"Katalog kapsam metriği üretilemiyor");
expect(Number.isFinite(metrics.quality.modelIssue),"Model-ariza kapsam metriği üretilemiyor");
...
console.log(`Admin dashboard doğrulandı: ${metrics.counts.deviceTypes} cihaz, ${metrics.counts.brands} marka, ${metrics.counts.models} model, ${metrics.counts.issues} arıza; placeholder=0.`);
