...
function analyticsResponse(request,response,env) {
  ...
  if (url.pathname==="/admin"||url.pathname.startsWith("/admin/")||!contentType.includes("text/html")) return response;
  ...
  const consentScript = `<script>(function(){
    var GA=${JSON.stringify(ga)};var CLARITY=${JSON.stringify(clarity)};var KEY='arizanerede_analytics_consent';
    function getChoice(){try{return localStorage.getItem(KEY)||''}catch(e){return ''}}function setChoice(v){try{localStorage.setItem(KEY,v)}catch(e){}}
    function enableGA(){if(GA)window['ga-disable-'+GA]=false}function disableGA(){if(GA)window['ga-disable-'+GA]=true}
    function loadGA(){if(!GA||window.__anGaLoaded)return;enableGA();window.__anGaLoaded=true;var s=document.createElement('script');s.async=true;s.src='https://www.googletagmanager.com/gtag/js?id='+encodeURIComponent(GA);document.head.appendChild(s);window.dataLayer=window.dataLayer||[];window.gtag=function(){dataLayer.push(arguments)};gtag('js',new Date());gtag('config',GA,{'anonymize_ip':true});}
    ...
    function start(){enableGA();loadGA();loadClarity();if(window.clarity)window.clarity('consentv2',{ad_Storage:'denied',analytics_Storage:'granted'})}function show(){var b=document.getElementById('an-consent');if(b)b.hidden=false}function hide(){var b=document.getElementById('an-consent');if(b)b.hidden=true}
    window.__anConsentAccept=function(){setChoice('granted');hide();start()};window.__anConsentReject=function(){setChoice('denied');hide();disableGA();if(window.clarity)window.clarity('consentv2',{ad_Storage:'denied',analytics_Storage:'denied'})};window.__anConsentSettings=function(){setChoice('');show()};
    document.addEventListener('DOMContentLoaded',function(){var c=getChoice();if(c==='granted')start();else{disableGA();if(c!=='denied')show()}});
  })();</script>`;
...
export default {
  async fetch(request, env, ctx) {
    ...
    return analyticsResponse(request,response,env);
  }
};
