...
function problemChips(items){return items.length?`<div class="problemBox"><span class="kicker">Araştırılan sorun alanları</span><h2>Bu grupta hangi arızalar ele alınıyor?</h2><div class="problemChips">${items.map(item=>`<span class="problemChip">${esc(item)}</span>`).join("")}</div></div>`:""}
...
export function renderBrand(device,brand){
  ...
  const directIssues=issues.filter(issue=>issue.deviceType===device.slug&&issue.brand===brand.slug);
  ...
  const summary=`<div class="answer"><strong>${publishedModels.length} tam model · ${directIssues.length} bağımsız hata/arıza sayfası</strong><span>Bu merkez model kataloğundan çok arıza, hata kodu, kılavuz, yazılım ve resmî servis erişimi için düzenlenir.</span></div>`;
  return page(`${brand.name} ${device.name} arızaları, hata kodları, kılavuz ve yetkili servis | Arıza Nerede?`,`${brand.name} ${device.name} arızaları, hata kodları, resmî kılavuz, yazılım/sürücü ve marka doğrulamalı yetkili servis kaynakları.`,`${SITE_ORIGIN}${pathForBrand(device.slug,brand.slug)}`,`<div class="shell">${crumb([{label:"Ana sayfa",url:"/"},{label:device.name,url:pathForDeviceType(device)},{label:brand.name}])}<section class="detail"><span class="eyebrow">${esc(device.name)} teknik destek merkezi</span><h1 class="title">${esc(brand.name)} ${esc(device.name)} arızaları ve destek</h1>${summary}<div class="cols"><article><p class="lead">Öncelik markanın kendi teknik verisidir. Hata kodları ve kullanıcı müdahalesi sınırları model bazında ayrılır; yetkili servis olarak yalnızca marka tarafından doğrulanan kanal gösterilir.</p>${problemChips(topProblems(publishedModels))}<section style="margin-top:22px"><h2>Seriler ve tam model aileleri</h2><div class="grid">${familyRows}</div></section></article><aside class="side">${brandSupport(brand)}<div class="card" style="margin-top:14px"><span class="kicker">Servis veya garanti sorunu</span><p>Servis kaydı, teslim belgesi, garanti veya onarım uyuşmazlığında haklarınızı ayrı rehberden kontrol edin.</p><a class="btn alt" href="/servis-garanti-haklari/">Servis ve tüketici hakları →</a></div></aside></div></section></div>`);
}
...
export function renderFamily(device,brand,family){
  ...
  const familyIssues=issues.filter(issue=>issue.deviceType===device.slug&&issue.brand===brand.slug&&issue.family===family.slug);
  ...
  return page(`${brand.name} ${family.name} arızaları, hata kodları ve kılavuz | Arıza Nerede?`,`${brand.name} ${family.name} tam modelleri, yaygın arızaları, hata kodları, kılavuz ve resmî teknik destek kaynakları.`,`${SITE_ORIGIN}${pathForFamily(family)}`,`<div class="shell">${crumb([{label:"Ana sayfa",url:"/"},{label:device.name,url:pathForDeviceType(device)},{label:brand.name,url:pathForBrand(device.slug,brand.slug)},{label:family.name}])}<section class="detail"><span class="eyebrow">${family.familyKind==="official-family"?"Resmî seri / aile":"Ürün grubu"}</span><h1 class="title">${esc(brand.name)} ${esc(family.name)} arızaları ve modelleri</h1><div class="answer"><strong>${familyModels.length} tam model · ${familyIssues.length} bağımsız hata/arıza sayfası</strong><span>Bu ailedeki modeller ortak sorun başlıkları yanında model-spesifik hata kodları ve resmî destek kaynaklarıyla ayrıştırılır.</span></div>${problemChips(topProblems(familyModels))}<h2 style="margin-top:24px">Tam modeller</h2><div class="grid">${rows}</div><h2 style="margin-top:26px">Teknik destek karşılaştırması</h2>${comparison}</section></div>`);
}
